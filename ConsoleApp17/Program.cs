...
class Program
{
    static void Main(string[] args)
    {
        ...
        // Test arithmetic checks using delegates
        int num = 8;

        Console.WriteLine($"{num} is even: {isEvenPredicate(num)}");
        Console.WriteLine($"{num} is odd: {isOddPredicate(num)}");
        Console.WriteLine($"{num} is prime: {isPrimePredicate(num)}");
        Console.WriteLine($"{num} is in Fibonacci sequence: {isFibonacciPredicate(num)}");
    }
}
